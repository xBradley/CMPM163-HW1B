Shader "CMPM163/Edge" {
    Properties {
		...
		_NumCells("Cell Number", Int) = 0
		_Pattern("StripesXY, StripesX, StripesY", Int) = 0
	...
	SubShader {
        ...
        Pass {
            ...
            fixed4 frag (vertexOutput i) : SV_Target {
				...
				//stripesXY
				if (_Pattern % 3 == 0) sum = floor(i.uv.x * _NumCells) + floor(i.uv.y * _NumCells);
				//stripesX
				else if (_Pattern % 3 == 1) sum = floor(i.uv.x * _NumCells); 
				//stripesY
				else if (_Pattern % 3 == 2) sum = floor(i.uv.y * _NumCells);
